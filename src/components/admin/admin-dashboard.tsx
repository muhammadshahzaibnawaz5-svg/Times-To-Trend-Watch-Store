...
export async function AdminDashboard() {
  ...
  try {
    ...
    console.log('[AdminDashboard] products count:', productCount);
  } catch (err) {
    console.error('[AdminDashboard] products query failed:', err);
    productCount = 0;
  }
  ...
  try {
    ...
    console.log('[AdminDashboard] orders count:', orderCount);
  } catch (err) {
    console.error('[AdminDashboard] orders query failed:', err);
    orderCount = 0;
  }
  ...
  try {
    ...
    console.log('[AdminDashboard] customer count:', customerCount);
  } catch (err) {
    console.error('[AdminDashboard] customers query failed:', err);
    customerCount = 0;
  }

  // --- Revenue data (last 30 days) ---
  let revenueData: { created_at: string | null; total_amount: unknown }[] = [];
  try {
    const result = await supabase
      .from('orders')
    ...
    revenueData = (result.data as any[]) || [];
    console.log('[AdminDashboard] revenue rows:', revenueData.length);
  } catch (err) {
    console.error('[AdminDashboard] revenue query failed:', err);
    revenueData = [];
  }
