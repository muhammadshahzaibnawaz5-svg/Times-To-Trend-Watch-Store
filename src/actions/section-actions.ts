...
export async function createSection(formData: FormData): Promise<ActionResult<Section>> {
  ...
  try {
    ...
    if (!type) {
      return { data: null, error: 'Section type is required' };
    }
    ...
    const msg = `[createSection] UNEXPECTED ERROR: ${err instanceof Error ? err.message : String(err)}`;
    console.error(msg);
...
export async function updateSection(id: string, formData: FormData): Promise<ActionResult<Section>> {
  console.log('[updateSection] called id=', id);
  try {
    await requireAdmin();
    const supabase = createAdminClient();
    const service = new SectionService(supabase);
    ...
    const msg = `[updateSection] UNEXPECTED ERROR: ${err instanceof Error ? err.message : String(err)}`;
    console.error(msg);
    return { data: null, error: msg };
  }
}
...
export async function deleteSection(id: string): Promise<ActionResult<null>> {
  console.log('[deleteSection] called id=', id);
  try {
    await requireAdmin();
    const supabase = createAdminClient();
    const service = new SectionService(supabase);
    ...
    const msg = `[deleteSection] UNEXPECTED ERROR: ${err instanceof Error ? err.message : String(err)}`;
    console.error(msg);
    return { data: null, error: msg };
  }
}
